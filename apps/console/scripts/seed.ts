...
import { consoleUsers, demoProducts, knowledgeDocuments } from "../src/db/schema";
...
const [knowledge] = await db.select({ id: knowledgeDocuments.id }).from(knowledgeDocuments).where(eq(knowledgeDocuments.title, "演示业务服务说明")).limit(1);
if (!knowledge) {
